...
module.exports = ({ pg, db }) => async (err, req, res, next) => {
  ...
  switch (true) {
    ...
    case Boolean(url.match("^/api/achievements/new/?$")) && method === "GET": {
      if (!req.isAuthenticated()) return res.sendStatus(401);
    ...
    case Boolean(url.match("^/api/achievements/new/?$")) && method === "PUT": {
      if (!req.isAuthenticated()) return res.sendStatus(401);
      const { name = "" } = req.body;
